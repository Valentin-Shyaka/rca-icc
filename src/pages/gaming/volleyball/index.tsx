...
const BasketballGaming = () => {
  ...
  const basketballMatchNS = matches?.filter(
    (match) => match?.status.status === 'NS' && match?.category === 'volleyball',
  );
  return (
    <GamingLayout>
      <div>
        ...
        {basketballMatchNS?.length == 0 && (
          <h1 className="font-bold text-center align-middle">There's no more matches to predict for VolleyBall</h1>
        )}
      </div>
    </GamingLayout>
  );
