...
const PlayerLineUp = ({ awayLineUp, homeLineUp }: Props) => {
	...
	return (
		<>
			{homeLineUp.eleven?.length > 0 ? (<div className='h-1/2 w-full flex-col flex p-2 justify-between'>
				...
				<div className={`grid grid-cols-${homeDefenders.length} w-full`} style={{ gridTemplateColumns: `repeat(${homeDefenders.length}, 1fr)` }}
				>
					{homeDefenders.map((player, i) => (
						<PlayerBox key={i} name='Player 2' color={`#ff7b35`} {...player} id={i} />
					))}
				</div>
				<div className={`grid grid-cols-${homeMidfielders.length} w-full`} style={{ gridTemplateColumns: `repeat(${homeMidfielders.length}, 1fr)` }}
				>
					{homeMidfielders.map((player, i) => (
						<PlayerBox key={i} name='Player 6' color={`#ff7b35`} {...player} id={i} />
					))}
				</div>
				<div className={`grid grid-cols-${homeForwards.length} w-full`} style={{ gridTemplateColumns: `repeat(${homeForwards.length}, 1fr)` }}
				>
			...
			{awayLineUp.eleven?.length > 0 ? (
				<div className='h-1/2 w-full flex-col-reverse flex p-2 justify-between'>
					...
					<div className={`grid grid-cols-${awayDefenders.length} w-full`}
...
export const PlayerBox = ({ name, number, color, id, displayName }: any) => (
