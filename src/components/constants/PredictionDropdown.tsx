...
import { playersByTeamQuery } from '../../lib/queries'
import { FaPlus } from "react-icons/fa"
import {  Player, Team } from "../../utils/types/types1";
  ...
  return (
    <div className=' max-sm:w-full h-fit p-2 border-b border-slate-300 cursor-pointer  '>
        <div className='flex justify-between items-center'>
         ...
         {dropActive?
         <div className=''>
            <BiSolidChevronUp onClick={()=>{setDropActive(!dropActive)}} className=' font-bold '/>
         </div>
         :
         <div className='bg-[#2076F8] p-1 rounded-lg'>
         <FaPlus onClick={()=>{setDropActive(!dropActive)}} className=' font-bold text-sm text-white'/>
         </div>
         }
        </div>
        {dropActive?
